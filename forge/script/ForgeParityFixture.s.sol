...
contract ForgeParityFixture is Script {
    string internal constant SCENARIO_ALL = "all";
    string internal constant SCENARIO_NO_CROSS = "no-cross";
    string internal constant SCENARIO_CROSS_ONE_TICK = "cross-one-tick";
    string internal constant SCENARIO_EXACT_OUTPUT = "exact-output";
    string internal constant SCENARIO_PROTOCOL_FEE = "protocol-fee";
    string internal constant SCENARIO_ONE_FOR_ZERO_CROSS = "one-for-zero-cross";
    bytes1 internal constant COMMA = 0x2c;

    function run() external {
        string memory scenarioList = vm.envOr(
            "FORGE_PARITY_SCENARIO",
            SCENARIO_ALL
        );
        string memory out = "[";
        bool wrote = false;

        (out, wrote) = appendScenario(
            out,
            wrote,
            scenarioList,
            SCENARIO_NO_CROSS
        ...
        (out, wrote) = appendScenario(
            ...
            wrote,
            scenarioList,
            SCENARIO_CROSS_ONE_TICK
        );
        (out, wrote) = appendScenario(
            out,
            wrote,
            scenarioList,
            SCENARIO_EXACT_OUTPUT
        );
        (out, wrote) = appendScenario(
            out,
            wrote,
            scenarioList,
            SCENARIO_PROTOCOL_FEE
        );
        (out, wrote) = appendScenario(
            out,
            wrote,
            scenarioList,
            SCENARIO_ONE_FOR_ZERO_CROSS
        );
        ...
        returns (ForgeParityFixtureHarness.Config memory c)
    {
        ...
        c.liquidityDelta = checkedInt128(
            vm.envOr("LIQUIDITY_DELTA", uint256(1_000_000))
        );
    ...
    function applyScenarioPreset(
        ForgeParityFixtureHarness.Config memory c,
        string memory scenario
    ) internal view {
        c.scenario = scenario;

        if (eq(scenario, SCENARIO_NO_CROSS)) {
            applyNoCross(c);
        } else if (eq(scenario, SCENARIO_CROSS_ONE_TICK)) {
            applyCrossOneTick(c);
        } else if (eq(scenario, SCENARIO_EXACT_OUTPUT)) {
            applyExactOutput(c);
        } else if (eq(scenario, SCENARIO_PROTOCOL_FEE)) {
            applyProtocolFee(c);
        } else if (eq(scenario, SCENARIO_ONE_FOR_ZERO_CROSS)) {
            applyOneForZeroCross(c);
        } else {
    ...
    function applyScenarioOverrides(
        ...
        if (vm.envExists("TICK_LOWER_OFFSET"))
            c.tickLowerOffset = envUint24("TICK_LOWER_OFFSET");

        if (vm.envExists("TICK_UPPER_OFFSET"))
            c.tickUpperOffset = envUint24("TICK_UPPER_OFFSET");

        if (vm.envExists("SWAP_AMOUNT"))
            c.swapAmount = vm.envUint("SWAP_AMOUNT");

        if (vm.envExists("PROTOCOL_FEE"))
            c.protocolFee = envUint24("PROTOCOL_FEE");

        if (vm.envExists("SWAP_EXACT_OUTPUT"))
            c.exactOutput = vm.envBool("SWAP_EXACT_OUTPUT");
    }

    function envUint24(string memory name) internal view returns (uint24) {
        uint256 value = vm.envUint(name);
        require(value <= type(uint24).max, "env uint24 overflow");
        // forge-lint: disable-next-line(unsafe-typecast)
        return uint24(value);
    }

    function envPositiveInt24(
        string memory name
    ) internal view returns (int24) {
        uint256 value = vm.envUint(name);
        require(
            value <= uint256(uint24(type(int24).max)),
            "env int24 overflow"
        );
        // forge-lint: disable-next-line(unsafe-typecast)
        return int24(int256(value));
    }

    function checkedInt128(uint256 value) internal pure returns (int128) {
        require(
            value <= uint256(uint128(type(int128).max)),
            "env int128 overflow"
        );
        // forge-lint: disable-next-line(unsafe-typecast)
...
contract ForgeParityFixtureHarness is Script {
    uint160 internal constant MIN_SQRT_PRICE = 4_295_128_739;
    uint160 internal constant MAX_SQRT_PRICE =
        1_461_446_703_485_210_103_287_273_052_203_988_822_378_723_970_342;
    uint8 internal constant ACTION_MINT = 1;
    uint8 internal constant ACTION_SWAP = 2;
    uint8 internal constant ACTION_COLLECT = 3;
    uint8 internal constant ACTION_DECREASE = 4;
    ...
    function run(Config memory input) external returns (string memory) {
        ...
        IPoolManagerLike(cfg.poolManager).unlock(abi.encode(ACTION_MINT));
        IPoolManagerLike(cfg.poolManager).unlock(abi.encode(ACTION_SWAP));
        IPoolManagerLike(cfg.poolManager).unlock(abi.encode(ACTION_COLLECT));
        IPoolManagerLike(cfg.poolManager).unlock(abi.encode(ACTION_DECREASE));
    ...
    function validateConfiguredContracts() internal view {
        require(
            cfg.poolManager.code.length != 0,
            "POOL_MANAGER has no code at fork block"
        );
        if (cfg.currency0 != address(0)) {
            require(
                cfg.currency0.code.length != 0,
                "CURRENCY0 has no code at fork block"
            );
        }
        if (cfg.currency1 != address(0)) {
            require(
                cfg.currency1.code.length != 0,
                "CURRENCY1 has no code at fork block"
            );
        }
    }
    ...
    function applyProtocolFeeOverride() internal {
        ...
        patchSlot0ProtocolFee(cfg.protocolFee, cfg.zeroForOne);
    }

    function patchSlot0ProtocolFee(
        uint24 protocolFee,
        bool zeroForOne
    ) internal {
        bytes32 slot = poolStateSlot();
        uint256 slot0 = uint256(
            IPoolManagerLike(cfg.poolManager).extsload(slot)
        );
        uint256 clearProtocolFeeMask = ~(uint256(0xFFFFFF) << 184);
        uint256 packedProtocolFee = zeroForOne
            ? uint256(protocolFee)
            : uint256(protocolFee) << 12;
        uint256 updated = (slot0 & clearProtocolFeeMask) |
            (packedProtocolFee << 184);
    ...
    function unlockCallback(
        ...
        if (action == ACTION_MINT) {
            (int256 callerDelta, int256 fees) = IPoolManagerLike(
                cfg.poolManager
            ).modifyLiquidity(
                    key,
            ...
            fillPositionStep(
                mintResult,
                principalDelta(callerDelta, fees),
                fees
            );
        } else if (action == ACTION_SWAP) {
            uint160 limit = cfg.zeroForOne
                ? MIN_SQRT_PRICE + 1
                : MAX_SQRT_PRICE - 1;
            int256 amountSpecified = cfg.exactOutput
                ? int256(cfg.swapAmount)
                : -int256(cfg.swapAmount);
            int256 swapDelta = IPoolManagerLike(cfg.poolManager).swap(
                key,
    ...
    function resolveTickRangeFromCurrentTick() internal {
        ...
        require(
            cfg.tickLower < currentTick,
            "lower must be below current tick"
        );
        require(
            cfg.tickUpper > currentTick,
            "upper must be above current tick"
        );
        require(cfg.tickLower < cfg.tickUpper, "invalid tick range");
    }

    function floorToSpacing(
        int24 tick,
        int24 spacing
    ...
    function fundHarness(
        ...
        vm.startPrank(whale);
        require(
            IERC20Minimal(token).transfer(address(this), resolvedAmount),
            "fund transfer failed"
        );
        vm.stopPrank();
    }

    function resolveFundAmount(
        address token,
        address whale,
    ...
    function settleCurrency(address currency, int128 amount) internal {
        if (amount < 0) {
            ...
            if (currency == address(0)) {
                ...
                require(
                    IERC20Minimal(currency).transfer(cfg.poolManager, owed),
                    "settle transfer failed"
                );
                manager.settle();
            }
        } else if (amount > 0) {
            IPoolManagerLike(cfg.poolManager).take(
                currency,
                address(this),
                positiveI128ToU128(amount)
            );
        }
    }

    function fillPositionStep(
        StepResult storage result,
        int256 principal,
        int256 feeDelta
    ) internal {
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    u(feeGrowthGlobal0X128),
                    '","fee_growth_global1_x128":"',
                    u(feeGrowthGlobal1X128),
                    '"',
                    '},"fee":',
                    u(lpFee),
                    ',"tick_spacing":',
    ...
    {
        bytes32 data = IPoolManagerLike(cfg.poolManager).extsload(
            poolStateSlot()
        );
    ...
    function getLiquidity() internal view returns (uint128 liquidity) {
        ...
        liquidity = uint128(
            uint256(IPoolManagerLike(cfg.poolManager).extsload(slot))
        );
    }
        ...
        returns (uint256 feeGrowthGlobal0X128, uint256 feeGrowthGlobal1X128)
    {
        bytes32 slot = bytes32(
            uint256(poolStateSlot()) + FEE_GROWTH_GLOBAL0_OFFSET
        );
        bytes32[] memory values = IPoolManagerLike(cfg.poolManager).extsload(
            slot,
            2
        );
    ...
    {
        bytes32[] memory data = IPoolManagerLike(cfg.poolManager).extsload(
            tickInfoSlot(targetTick),
            3
        );
    ...
    function tickInfoSlot(int24 targetTick) internal view returns (bytes32) {
        bytes32 ticksMappingSlot = bytes32(
            uint256(poolStateSlot()) + TICKS_OFFSET
        );
        return
            keccak256(abi.encodePacked(int256(targetTick), ticksMappingSlot));
    }
        ...
        if (lowerGross != 0) {
            out = string(
                abi.encodePacked(
                    out,
                    wrote ? "," : "",
                    tickJson(cfg.tickLower, lowerGross, lowerNet)
                )
            );
            wrote = true;
        }
        if (upperGross != 0) {
            out = string(
                abi.encodePacked(
                    out,
                    wrote ? "," : "",
                    tickJson(cfg.tickUpper, upperGross, upperNet)
                )
            );
        }
    ...
    function absI128(int128 value) internal pure returns (uint128) {
        int256 wide = int256(value);
        uint256 magnitude = uint256(wide < 0 ? -wide : wide);
        require(magnitude <= type(uint128).max, "i128 abs overflow");
        // forge-lint: disable-next-line(unsafe-typecast)
        return uint128(magnitude);
    }

    function positiveI128ToU128(int128 value) internal pure returns (uint128) {
        require(value > 0, "expected positive i128");
        // forge-lint: disable-next-line(unsafe-typecast)
        return uint128(value);
    }

    function checkedI256ToI128(int256 value) internal pure returns (int128) {
        require(
            value >= type(int128).min && value <= type(int128).max,
            "i256 to i128 overflow"
        );
        // forge-lint: disable-next-line(unsafe-typecast)
        return int128(value);
    }
    ...
    function fixtureJson(
        string memory initialPool,
        string memory finalPool
    ) internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
    ...
    function protocolFeeOperationJson() internal view returns (string memory) {
        if (cfg.protocolFee == 0) return "";
        return string(abi.encodePacked(',"protocol_fee":', u(cfg.protocolFee)));
    }
    ...
    function collectOperationJson() internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '{"type":"collect","token_id":1,"expect":{',
                    modifyExpectJson(collectResult),
                    "}}"
                )
            );
    }
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    deltaJson(d0, d1),
                    ',"sqrt_price_x96":"',
                    u(result.sqrtPriceX96),
                    '","tick":',
                    s(result.tick),
                    ',"liquidity":',
                    u(result.liquidity),
                    ',"fee_growth_global0_x128":"',
                    u(result.feeGrowthGlobal0X128),
                    '","fee_growth_global1_x128":"',
                    u(result.feeGrowthGlobal1X128),
                    '"}'
                )
            );
    }
    ...
    function s(int256 value) internal pure returns (string memory) {
        return vm.toString(value);
    }

    function a(address value) internal pure returns (string memory) {
        return vm.toString(value);
    }

    function b(bool value) internal pure returns (string memory) {
        return value ? "true" : "false";
    }

    function unpackDelta(
        int256 delta
    ) internal pure returns (int128 amount0, int128 amount1) {
        // V4 packs BalanceDelta as int128 amount0 in the high word and int128 amount1 in the low word.
        // forge-lint: disable-next-line(unsafe-typecast)
